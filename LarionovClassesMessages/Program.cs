        ...
        static void Main(string[] args)
        {
            ...
            while (IsGo)
            {
                ...
                while (IsFind)
                {
                    ...
                    myPrint.PrintWarning($"Введите имя резервуара (чувствительно к регистру) чтобы получить по нему информацию\n");
                    ...
                    if (!IsQuestion("Хотите повторить поиск с текущими исходными данными? [y/n] (по умолчанию y): "))
                        IsFind = false;
                }
                if (IsQuestion("Хотите выйти? [y/n] (по умолчанию y): "))
                    IsGo = false;
            }
        }
    }
